...
class CustomMany extends Relation {
    /**
    ...
    */
    public function addConstraints() {
        if (static::$constraints) call_user_func($this->baseConstraints, $this);
    }
    ...
    */
    public function initRelation(array $models, $relation) {
        foreach ($models as $model) $model->setRelation($relation, $this->related->newCollection());
    ...
    */
    public function get($columns = ['*']) {
        ...
        if ($columns == ['*']) $columns = [$this->related->getTable().'.*'];
        ...
        if (count($models) > 0) $models = $builder->eagerLoadRelations($models);
